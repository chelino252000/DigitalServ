...
@Composable
fun DeleteComputerScreen(navController: NavController) {
    val context = LocalContext.current
    val db = Firebase.firestore // Instancia de Firestore

    // Estado para el número de serie
    val serialNumber = remember { mutableStateOf("") }

    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                title = { Text("Eliminar Computadora", color = Color.White) },
                navigationIcon = {
    ...
    Column(
        ...
        Text(text = "Eliminar registro de computadora", fontSize = 16.sp, fontWeight = FontWeight.Bold)
        ...
        Button(
            onClick = {
                if (serialNumber.value.isNotBlank()) {
                        ...
                        .addOnSuccessListener { documents ->
                            if (!documents.isEmpty) {
                                for (document in documents) {
                                    db.collection("computers").document(document.id)
                                        .delete()
                                        .addOnSuccessListener {
                                            Toast.makeText(context, "Computadora eliminada con éxito", Toast.LENGTH_SHORT).show()
                                ...
                                Toast.makeText(context, "No se encontró una computadora con ese número de serie", Toast.LENGTH_SHORT).show()
                            }
                        }
                        .addOnFailureListener { e ->
                    ...
                    Toast.makeText(context, "Por favor, ingresa el número de serie", Toast.LENGTH_SHORT).show()
                }
            },
