...
@Composable
fun ComputerUpdateScreen(navController: NavController) {
    ...
    val isEditing = remember { mutableStateOf(false) } // Estado para indicar si se encontró el equipo
    val documentId = remember { mutableStateOf("") } // ID del documento de Firestore

    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                title = { Text("Actualizar Computadora", color = Color.White) },
                navigationIcon = {
    ...
    Column(
        ...
        if (!isEditing.value) {
            Text(text = "Buscar equipo por número serial", fontSize = 16.sp, fontWeight = FontWeight.Bold)
                        ...
                        db.collection("computers")
                            ...
                            .addOnSuccessListener { documents ->
                                if (documents.isEmpty) {
                                    Toast.makeText(context, "No se encontró el equipo", Toast.LENGTH_SHORT).show()
                                } else {
            ...
            Text(text = "Actualizar datos del equipo", fontSize = 16.sp, fontWeight = FontWeight.Bold)
            Spacer(modifier = Modifier.height(16.dp))
