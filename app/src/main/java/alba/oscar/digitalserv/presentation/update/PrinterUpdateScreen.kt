...
@Composable
fun PrinterUpdateScreen(navController: NavController) {
    ...
    val isEditing = remember { mutableStateOf(false) } // Estado para indicar si se encontró la impresora
    val documentId = remember { mutableStateOf("") } // ID del documento de Firestore

    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                title = { Text("Actualizar impresora", color = Color.White) },
                navigationIcon = {
    ...
    Column(
        ...
        if (!isEditing.value) {
            Text(text = "Buscar la impresora por número serial", fontSize = 16.sp, fontWeight = FontWeight.Bold)
            ...
            Button(
                onClick = {
                    if (serialNumber.value.isNotBlank()) {
                        db.collection("printer") // Cambiado a la colección 'mice'
                            .whereEqualTo("serialNumber", serialNumber.value)
                            .get()
                            .addOnSuccessListener { documents ->
                                if (documents.isEmpty) {
                                    Toast.makeText(context, "No se encontró la impresora", Toast.LENGTH_SHORT).show()
                                } else {
            ...
            Text(text = "Actualizar datos de la impresora", fontSize = 16.sp, fontWeight = FontWeight.Bold)
            Spacer(modifier = Modifier.height(16.dp))
