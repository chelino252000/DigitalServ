...
@Composable
fun PrinterDeleteScreen(navController: NavController) {
    ...
    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                title = { Text("Eliminar Impresora", color = Color.White) },
                navigationIcon = {
    ...
    Column(
        ...
        Text(text = "Eliminar registro de las impresoras", fontSize = 16.sp, fontWeight = FontWeight.Bold)
        ...
        Button(
            onClick = {
                if (serialNumber.value.isNotBlank()) {
                    db.collection("printer") // Cambiar la colección si es específica para "impresoras"
                        .whereEqualTo("serialNumber", serialNumber.value) // Busca por número de serie
                        .get()
                        .addOnSuccessListener { documents ->
                            if (!documents.isEmpty) {
                                for (document in documents) {
                                    db.collection("printer").document(document.id)
                                        .delete()
                                        .addOnSuccessListener {
                                            Toast.makeText(context, "Impresora eliminada correctamente", Toast.LENGTH_SHORT).show()
                                ...
                                Toast.makeText(context, "No se encontró una impresora con ese número serial", Toast.LENGTH_SHORT).show()
                            }
                        }
                        .addOnFailureListener { e ->
