...
@Composable
fun KeyboardDeleteScreen(navController: NavController) {
    ...
    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                title = { Text("Eliminar Teclado", color = Color.White) },
                navigationIcon = {
    ...
    Column(
        ...
        Text(text = "Eliminar registro del teclado", fontSize = 16.sp, fontWeight = FontWeight.Bold)
        ...
        Button(
            onClick = {
                if (serialNumber.value.isNotBlank()) {
                    db.collection("Keyboards") // Cambiar la colección si es específica para "teclados"
                        .whereEqualTo("serialNumber", serialNumber.value) // Busca por número de serie
                        .get()
                        .addOnSuccessListener { documents ->
                            if (!documents.isEmpty) {
                                for (document in documents) {
                                    db.collection("Keyboards").document(document.id)
                                        .delete()
                                        .addOnSuccessListener {
                                            Toast.makeText(context, "Teclado eliminado correctamente", Toast.LENGTH_SHORT).show()
                                ...
                                Toast.makeText(context, "No se encontró un teclado con ese número serial", Toast.LENGTH_SHORT).show()
                            }
                        }
                        .addOnFailureListener { e ->
