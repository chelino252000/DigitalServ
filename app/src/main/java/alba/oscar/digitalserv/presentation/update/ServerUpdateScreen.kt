...
@Composable
fun ServerUpdateScreen(navController: NavController) {
    ...
    val isEditing = remember { mutableStateOf(false) } // Estado para indicar si se encontró el servidor
    val documentId = remember { mutableStateOf("") } // ID del documento de Firestore

    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                title = { Text("Actualizar el servidor", color = Color.White) },
                navigationIcon = {
    ...
    Column(
        ...
        if (!isEditing.value) {
            Text(text = "Buscar servidor por número serial", fontSize = 16.sp, fontWeight = FontWeight.Bold)
            ...
            Button(
                onClick = {
                    if (serialNumber.value.isNotBlank()) {
                        db.collection("server") // Cambiado a la colección 'mice'
                            .whereEqualTo("serialNumber", serialNumber.value)
                            .get()
                            .addOnSuccessListener { documents ->
                                if (documents.isEmpty) {
                                    Toast.makeText(context, "No se encontró el servidor", Toast.LENGTH_SHORT).show()
                                } else {
            ...
            Text(text = "Actualizar datos del servidor", fontSize = 16.sp, fontWeight = FontWeight.Bold)
            Spacer(modifier = Modifier.height(16.dp))
