...
@Composable
fun MouseDeleteScreen(navController: NavController) {
    ...
    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                title = { Text("Eliminar Mouse", color = Color.White) },
                navigationIcon = {
    ...
    Column(
        ...
        Text(text = "Eliminar registro del mouse", fontSize = 16.sp, fontWeight = FontWeight.Bold)
        ...
        Button(
            onClick = {
                if (serialNumber.value.isNotBlank()) {
                    db.collection("mouse") // Cambiar la colección si es específica para "mouses"
                        .whereEqualTo("serialNumber", serialNumber.value) // Busca por número de serie
                        .get()
                        .addOnSuccessListener { documents ->
                            if (!documents.isEmpty) {
                                for (document in documents) {
                                    db.collection("mouse").document(document.id)
                                        .delete()
                                        .addOnSuccessListener {
                                            Toast.makeText(context, "Mouse eliminado correctamente", Toast.LENGTH_SHORT).show()
                                ...
                                Toast.makeText(context, "No se encontró un mouse con ese número serial", Toast.LENGTH_SHORT).show()
                            }
                        }
                        .addOnFailureListener { e ->
