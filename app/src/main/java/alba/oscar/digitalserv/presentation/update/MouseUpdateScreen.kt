...
@Composable
fun MouseUpdateScreen(navController: NavController) {
    ...
    val isEditing = remember { mutableStateOf(false) } // Estado para indicar si se encontró el mouse
    val documentId = remember { mutableStateOf("") } // ID del documento de Firestore

    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(
                title = { Text("Actualizar Mouse", color = Color.White) },
                navigationIcon = {
                    IconButton(onClick = { navController.navigate("mouse_Screen") }) {
                        Icon(
    ...
    Column(
        ...
        if (!isEditing.value) {
            Text(text = "Buscar mouse por número serial", fontSize = 16.sp, fontWeight = FontWeight.Bold)
            ...
            Button(
                onClick = {
                    if (serialNumber.value.isNotBlank()) {
                        db.collection("mouse") // Cambiado a la colección 'mice'
                            .whereEqualTo("serialNumber", serialNumber.value)
                            .get()
                            .addOnSuccessListener { documents ->
                                if (documents.isEmpty) {
                                    Toast.makeText(context, "No se encontró el mouse", Toast.LENGTH_SHORT).show()
                                } else {
            ...
            Text(text = "Actualizar datos del mouse", fontSize = 16.sp, fontWeight = FontWeight.Bold)
            Spacer(modifier = Modifier.height(16.dp))
